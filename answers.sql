Write an SQL query to retrieve the checkNumber, paymentDate, and amount from the payments table.
  Ans: SELECT checkNumber, paymentDate, amount 
      FROM payments;
...
Ans: SELECT orderDate, requiredDate, status 
     FROM orders 
...
Ans: SELECT productName, quantityInStock 
     FROM products 
